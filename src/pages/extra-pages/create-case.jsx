...
export default function AddCase() {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const response = await fetch('http://localhost:5000/api/litigations/create', {
        method: 'POST',
        headers: {
      ...
      if (response.ok) {
        // Handle successful creation (e.g., navigate to cases list or show success message)
    ...
    <Grid container spacing={3}>
      <Grid item xs={12}>
        <Stack direction="row" justifyContent="space-between" alignItems="baseline" sx={{ mb: { xs: -0.5, sm: 0.5 } }}>
          <Typography variant="h3">Add New Case</Typography>
